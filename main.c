...
void affiche_tout(game *myGame,font mFont, int score1, int score2, SDL_Rect j1, SDL_Rect j2, balle_jeu balle);
void affiche_balle(game *myGame,int x0, int y0, int radius);
void affiche_score(game *myGame, font mFont, int score1, int score2);
void affiche_raquette(game *myGame, SDL_Rect j1, SDL_Rect j2);
...
int main(int argc, char *argv[])
{
    ...
    if(init("SDL_Pong",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,SCREEN_WIDTH,SCREEN_HEIGHT,SDL_WINDOW_SHOWN,&myGame,&mFont, &j1, &j2, &score1, &score2, &balle)){
        state.g_bRunning=1;
    }else{
    ...
    while(state.g_bRunning){
            ...
            affiche_tout(&myGame,mFont,score1,score2,j1,j2,balle);
...
/*
BUT : Lire les touches au clavier et mettre à jour les coordonnees des raquettes suite a l'appui.
ENTREE : La fenetre de jeu, l'etat du jeu, les coordonnees des raquettes
SORTIE : Les coordonnees de la raquette mises a jour suite a l'appui d'une touche
...
SORTIE : L'affichage de tous les éléments du renderer

*/

void affiche_tout(game *myGame,font mFont,int score1, int score2, SDL_Rect j1, SDL_Rect j2, balle_jeu balle) {

        affiche_score(myGame, mFont, score1, score2);

        //Affichage raquettes
        SDL_SetRenderDrawColor(myGame->g_pRenderer, 255, 255, 255, SDL_ALPHA_OPAQUE);
        affiche_raquette(myGame,j1,j2);
...
/*
BUT : Afficher les raquettes
ENTREE : Le jeu, les rectangles des joueurs
SORTIE : Enregistrement des raquettes dans le renderer
...
/*
BUT : Gerer l'intervalle d'affichage entre 2 images du jeu
...
/*
BUT : Liberer la memoire en fin de programme
ENTREE : La fenetre de jeu, la police
SORTIE : La memoire liberee
...
SORTIE : Affichage direct de la balle
...
ENTREE : Le jeu, la police/font du score, score du joueur 1, score du joueur 2
SORTIE : Enregistrement du score dans le renderer
...
/*
BUT : Donner la prochaine position de la balle en fonction de son emplacement
ENTREE : La balle, les raquettes et les scores (en cas de marquage de points)
SORTIE : Coordonnees de la balle mises a jour, mise a jour eventuelle des scores
...
/*
BUT : Initialiser la position, le rayon et la trajectoire de la balle
ENTREE : La balle
SORTIE : Les donnees initialisees (position au centre et rayon=10)
...
/*
BUT : Initialiser la position, la longueur et la largeur des raquettes
ENTREE : Les raquettes des joueurs 1 et 2
SORTIE : Les donnees initialisees

*/

void init_raquettes(SDL_Rect *j1, SDL_Rect *j2){
    j1->x=20;
    j1->y=SCREEN_HEIGHT/2;
    j1->h=100;
    j1->w=20;

    j2->x=SCREEN_WIDTH-40;
    j2->y=SCREEN_HEIGHT/2;
    j2->h=100;
    j2->w=20;
}
